(function() {
	...
	/**
	 * Checks that element contains bloxk
	...
	BX.Landing.Main.prototype = {
		...
		initEmptyArea: function(area)
		{
			if (area)
			{
				area.innerHTML = "";
				append(
					(new PlusButton("insert_first_block", {
		...
		/**
		 * Uninitializes empty area
		 * @param {HTMLElement} area
		 */
		unInitEmptyArea: function(area)
		{
			if (area)
			{
		...
		adjustEmptyAreas: function()
		{
			this.getLayoutAreas()
				...
				.forEach(this.unInitEmptyArea, this);
		...
		appendBlock: function(data, withoutAnimation)
		{
			...
			if (this.currentBlock && this.currentBlock.node && this.currentBlock.node.parentNode)
			{
				insertAfter(block, this.currentBlock.node);
			}
			else
			{
				prepend(block, this.currentArea);
			}
		...
		showBlocksPanel: function(block, area)
		{
			...
			if (!this.blocksPanel)
			{
				...
				this.onBlocksListCategoryChange("popular");
		...
		getFeedbackFormOptions: function()
		{
			...
			if (hostName.includes(".ru") ||
				hostName.includes(".by") ||
				hostName.includes(".kz"))
			{
				return {"id": "8", "sec": "x80yjw", "lang": "ru"};
			}

			if (hostName.includes(".es"))
			{
				return {"id": "14", "sec": "wu561i", "lang": "la"};
			}

			if (hostName.includes(".de"))
			{
				return {"id": "10", "sec": "eraz2q", "lang": "de"};
			}

			if (hostName.includes(".com.br"))
			{
				return {"id": "12", "sec": "r6wvge", "lang": "br"};
			}

			if (hostName.includes(".ua"))
			{
				return {"id": "18", "sec": "d9e09o", "lang": "ua"};
			}

			return {"id": "16", "sec": "3h483y", "lang": "en"};
		},
		...
		onAddBlock: function(blockCode, restoreId, preventHistory)
		{
			...
			// Load block from server
			return this.loadBlock(blockCode, restoreId)
				.then(function(res) {
