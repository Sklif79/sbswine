...
if (!empty($arResult['PROPS_GROUPS'])) {
	foreach($arParams["PROPERTIES"] as $Key => $arProp) {
		if(in_array($Key,$arParams['EXCLUDE_PROPERTIES']) || (isset($arProp['DISPLAY_VALUE']) && strlen($arProp['DISPLAY_VALUE'])===0) || (!isset($arProp['DISPLAY_VALUE']) && is_array($arProp['VALUE']) && empty($arProp['VALUE'])) || (!isset($arProp['DISPLAY_VALUE']) && !is_array($arProp['VALUE']) && strlen($arProp['VALUE'])===0)) continue;
