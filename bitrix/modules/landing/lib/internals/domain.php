...
class DomainTable extends Entity\DataManager
{
	...
	protected static function prepareChange(Entity\Event $event)
	{
		...
		// valid protocol
		if (array_key_exists('PROTOCOL', $fields))
		{
			if (!self::isValidProtocol($fields['PROTOCOL']))
			{
				$result->setErrors(array(
					new Entity\EntityError(
						Loc::getMessage('LANDING_TABLE_ERROR_PROTOCOL_IS_NOT_VALID'),
						'PROTOCOL_IS_NOT_VALID'
					)
				));
				return $result;
			}
