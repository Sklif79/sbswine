...
class Landing extends \Bitrix\Landing\Internals\BaseTable
{
	 ...
	 * @return string
	 */
	public function getPublicUrl($id = false, $absolute = true)
	{
		if ($id === false)
		{
			$id = $this->id;
	...
	public function view(array $params = array())
	{
		...
		// breadcrumb (see chain_template.php in tpl)
		if ($this->mainInstance)
		{
			ob_start();
	...
	{
		// event for cancel publication
		$event = new Event('landing', 'onLandingPublication', array(
			'id' => $this->id
		));
		$event->send();
		foreach ($event->getResults() as $result)
