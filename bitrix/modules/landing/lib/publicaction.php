...
class PublicAction
{
	...
	public static function restBase()
	{
		...
		{
			$classes = array('site', 'landing', 'block', 'repo', 'template');
			...
			// placements
			$restMethods[\CRestUtil::PLACEMENTS] = array();
			$restMethods[\CRestUtil::PLACEMENTS]['LANDING_SETTINGS'] = array();
			$restMethods[\CRestUtil::PLACEMENTS]['LANDING_BLOCK_*'] = array();
			foreach (Block::getRepository() as $item)
			{
				foreach ($item['items'] as $key => $block)
				{
					$restMethods[\CRestUtil::PLACEMENTS]['LANDING_BLOCK_' . $key] = array();
				}
			}
		}
