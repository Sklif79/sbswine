						...
						onMarkerLoad = function onMarkerLoad(json) {
							...
							for (var i = 0; i < json.length; i++) {
								...
								var marker = new MarkerWithLabel({
									...
									icon: imgType,
									zIndex: 999999,
									labelContent: '<div id="content" style="white-space: nowrap;">' + '<div class="siteNotice">' + '<div class="map_adress">' + obj.name + '</div>' + '<div class="map_tell">' + obj.tell + '</div>' + '<div class="map_date">' + obj.dates + '</div>' + '</div>' + '</div>',
                            ...
                            for (var i2 = 0; i2 < json.length; i2++) {
                                var myLatLng = new google.maps.LatLng(json[i2]['lat'], json[i2]['lng']);
...
$(document).ready(function(){
    $(document).on('click', '.more_read', function(){
        ...
        {
            $('.desc_text').css('max-height', '9999px');
            $(this).text('Свернуть');
        }
        else
        {
            $('.desc_text').css('max-height', '280px');
