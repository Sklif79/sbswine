(function() {
		...
		BX.addCustomEvent(dialog.popup.popup, 'onPopupShow', function(obj)
		{
			...
			BX.bind(inp, 'keyup', BX.debounce(function()
			{
				...
				var fillInstruction = function(domainName)
				{
					...
					textNode1.textContent = domainName;
	...
	BX.Landing.Layout = function(params)
	{
		...
		var faviconFieldWrapper = document.querySelector('.landing-form-page-favicon');
		if (faviconFieldWrapper)
		{
			var faviconField = new BX.Landing.UI.Field.Image({
				content: {id: 'favicon'},
				disableAltField: true
			});
			faviconFieldWrapper.appendChild(faviconField.layout);
		}
