	...
	function showButtonLoader(button)
	{
		if (!!button)
		{
			if (!button.loader)
			{
				...
				style(button.loader.layout.querySelector(".main-ui-loader-svg-circle"), {
					"stroke-width": "4px"
				})
			}
	...
	BX.Landing.Block.prototype = {
		...
		onPlacementClick: function(placement)
		{
			BX.rest.AppLayout.openApplication(
				...
				{
					PLACEMENT: 'LANDING_BLOCK_' + this.manifest.code.toUpperCase(),
		...
		moveUp: function(preventHistory)
		{
			...
			if (prev)
			{
				...
				result.then(function() {
					style(current, {"transform": null, "transition": null});
					style(prev, {"transform": null, "transition": null});
		...
		moveDown: function(preventHistory)
		{
			...
			if (!!next)
			{
				...
				result.then(function() {
					style(current, {"transform": null, "transition": null});
					style(next, {"transform": null, "transition": null});
		...
		initCards: function()
		{
			this.forEachCard(function(node, selector, index) {
				...
				if (instance)
				{
					instance.panels.get("cardAction").layout.remove();
		...
		onNodeOptionsChange: function(data)
		{
			if (!isEmpty(data))
			{
				...
				return BX.Landing.Backend.getInstance()
					.action("Block::updateNode", data, queryParams);
			}
		},
		 ...
		 * @param {{name: string, nodes: BX.Landing.Collection.NodeCollection, [compact]: boolean}} [options]
		 */
					...
					type.forEach(function(type) {
						...
						var field = styleFactory.createField({
							...
							onChange: function(value, items, postfix, affect) {
								...
								this.onStyleInputWithDebounce({node: styleNode.node, data: styleNode.getValue()});
							}.bind(this)
						});
		...
		onAttributeChange: function(field)
		{
			...
			this.appendFieldValue(field, this.requestData);
		...
		appendFieldValue: function(field, requestData)
		{
		...
		saveChanges: function(data)
		{
			...
			{
				var requestData = {block: this.id, data: data, lid: this.lid, siteId: this.siteId};
				var queryParams = {code: this.manifest.code};

				return Backend.action("Landing\\Block::updateNodes", requestData, queryParams)
					.then(function() {
						return Promise.resolve(data);
					});
			}
		...
		fetchRequestData: function(panel)
		{
			...
			var settingsForms = new FormCollection();
			var requestData = {};

			panel.forms.forEach(function(form) {
				(form.type === "attrs" ? settingsForms : contentForms).add(form);
			});

			contentForms.fetchFields().fetchChanges().forEach(function(field) {
				requestData[field.selector] = field.getValue();
			}, this);

			settingsForms.fetchFields().fetchChanges().forEach(function(field) {
				var selector = this.makeAbsoluteSelector(field.selector);
				var value = field.getValue();
		...
		getEditForms: function(nodes, formName, nodesOnly)
		{
			...
			this.cards.forEach(function(card) {
				var cardForm = new CardForm({title: card.getName()});
